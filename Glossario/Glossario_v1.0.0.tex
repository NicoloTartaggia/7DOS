...
\newpage
	...
	\label{sec:dapps}
	Anche note come "dApps". Applicazioni o programmi che distribuiscono il calcolo su più terminali, invece di sfruttare una classica architettura client-server.
	...
	\label{sec:feeddati}
	File che contiene una lunga serie di informazioni, generalmente con estensione .xml o .csv; può essere associato a delle applicazioni, come i feed RSS.
	...
	\subsection{FreeLing}
	\label{sec:freeling}
	Suite di strumenti \underline{\hyperref[sec:opensource]{open source}} per l'analisi del linguaggio.
...
ISO 12207 definisce cosi' le fasi del ciclo di vita sequenziale: analisi, progettazione (intesa come \textit{design}), realizzazione, manutenzione.
	...
	\subsection{NoSQL}
	\label{sec:nosql}
	Acronimo di "non relational SQL", trattasi di un alternativa ai tradizionali database relazionali utile nel trattamento di grosse moli di dati e nell'utilizzo con applicazioni web real-time.
	\newpage
...
\newpage
	...
	\subsection{Swift}
	\label{sec:swift}
	Linguaggio di programmazione compilato multi-paradigma sviluppato dall'azienda statunitense Apple per scrivere programmi e applicazioni per i propri sistemi operativi.
	...
	\subsection{Truffle}
	\label{sec:truffle}
	Suite di strumenti per \underline{\hyperref[sec:smartcontracts]{smart contracts}} e per lo sviluppo di \underline{\hyperref[sec:dapps]{dApps}}.
