...
\tableofcontents
	...
	Data la misura del consumo di risorse che avviene nel compimento di un obiettivo, minore il consumo di risorse, maggiore \emph{l'efficienza}.
	...
	\subsection{Grafana}
	\label{sec:grafana}
	\emph{Grafana} è un compositore di grafici, che viene eseguito come applicazione web. Supporta \underline{\hyperref[sec:influxdb]{InfluxDB}}.


	\subsection{Grafana Plug-in Code Style Guide}
	\label{sec:grafana}
	Guida tecnica per la scrittura di codice per plug-in di \underline{\hyperref[sec:grafana]{Grafana}}.	
	...
	\subsection{HTML}
	\label{sec:html}
	In informatica \emph{HyperText Markup Language (HTML)} è un linguaggio di \emph{markup} nato per la formattazione e impaginazione di documenti ipertestuali disponibili nel web. \emph{HTML} è un linguaggio di pubblico dominio, la cui sintassi è stabilita dal \emph{World Wide Web Consortium} (W3C).
	...
	\label{sec:iee830}
	\underline{\hyperref[sec:bestpractice]{Best practice}} raccomandate per la specifica dei \underline{\hyperref[sec:requisito]{requisiti}} di \underline{\hyperref[sec:prodottosoftware]{prodotti software}}.
	...
	\subsection{InfluxDB}
	\label{sec:influxdb}
	\emph{InfluxDB} è un database di serie temporali \underline{\hyperref[sec:opensource]{open source}}. È ottimizzato per la memorizzazione e il recupero rapido di dati di serie temporali in campi come il monitoraggio delle operazioni, le metriche dell'applicazione, i dati di sensori e l'analisi in tempo reale.
	...
	Il termine \emph{in-schedule} indica la conformità alle aspettative temporali o di costi determinate in fase di pianificazione.
	...
	\item \emph{Functional Suitability}: idoneità funzionale, ossia quanto il prodotto rispetta i requisiti;
	\item \emph{Performance Efficiency}: \underline{\hyperref[sec:efficienza]{efficienza}} della \underline{\hyperref[sec:performance]{performance}}, ossia quanto il prodotto è in grado di sfruttare efficientemente le risorse disponibili;
	\item \emph{Compatibility}: compatibilità, ossia quanto il prodotto riesce ad interfacciarsi con altri prodotti o sistemi;
	\item \emph{Usability}: usabilità, ossia quanto il prodotto risulta di semplice utilizzo;
	\item \emph{Reliability}: affidabilità, ossia quanto il prodotto risulta stabile nell'esecuzione e nei risultati;
	\item \emph{Security}: sicurezza, ossia quanto il prodotto protegge i dati trattati;
	\item \emph{Maintainability}: manutenibilità, ossia quanto il prodotto si presta alla risoluzione di bug e problemi;
	\item \emph{Portability}: portabilità, ossia quanto il prodotto risulta utilizzabile su ambienti diversi tra loro.
	\end{itemize}
	...
	\item L'obiettivo di \emph{leadership} sul mercato;
	...
	\subsection{JPG}
	\label{sec:jpg}
	Formato \emph{lossy} per memorizzare immagini.


	\subsection{JavaScript}
	\label{sec:javascript}
	Linguaggio di \emph{scripting} orientato agli oggetti e agli eventi, comunemente utilizzato nella programmazione web, lato client, per la creazione di effetti dinamici interattivi tramite funzioni di \emph{script}, invocate da eventi innescati, a loro volta, dall'utente sulla pagina web in uso.
	...
	\subsection{LaTeX}
	\label{sec:latex}
	Linguaggio di \emph{markup} per la scrittura di testi, particolarmente usato in ambito accademico.
	...
	L'insieme delle funzioni amministrative, direttive e gestionali di un'impresa o di un'azienda. Il compito del \emph{manager}.
...
\underline{\hyperref[sec:iso12207]{ISO 12207}} definisce cosi' le fasi del ciclo di vita sequenziale: analisi, progettazione (intesa come \textit{design}), realizzazione, manutenzione.
		...
		\subsubsection{Evolutivo}
			Fa uso massiccio della fase di manutenzione. Viene eseguita ad ogni versione rilevante del prodotto.
		...
		\subsubsection{A spirale}
				Modello di ciclo di vita utilizzato quando il progetto è innovativo e non esistono \underline{\hyperref[sec:bestpractice]{best practice}} applicabili per lo sviluppo del progetto.
	...
	\subsection{Modulo}
	\label{sec:modulo}
	Si tratta di una componente o parte del software. \emph{Un modulo}:
	\begin{itemize}
	...
	\label{sec:smart}
	\begin{itemize}
			\item \emph{Specific}: formalmente definito;
			\item \emph{Measurable}: il cui stato di ottenimento è misurabile;
			\item \emph{Achievable}: raggiungibile;
			\item \emph{Realistic}: pratico;
			\item \emph{Time-Bound}: limitato da un vincolo temporale.
		\end{itemize}
	...
	\begin{itemize}
	\item Diagrammi di \emph{Gantt} (rappresentano durata prevista vs durata effettiva delle attività);
	...
	\subsection{Plug-in}
	\label{sec:plug-in}
	Un \emph{plug-in} in campo informatico è un programma non autonomo che interagisce con un altro programma per ampliare o estendere le funzionalità originarie. Ad esempio, un \emph{plug-in} per un software di grafica permette l'utilizzo di nuove funzioni non presenti nel software principale.
	...
	\subsection{Progetto}
	\label{sec:progetto}
	Insieme di attività e compiti atti a raggiungere \underline{\hyperref[sec:smart]{obiettivi SMART}}, che hanno una data di inizio ed una di fine e che consumano un \emph{pool} di risorse limitate.
	...
	\item Pianificare attività (organizzarle nel tempo e assegnarle);
	...
	\item Comprensibilità: facilità di utilizzo e comprensione del funzionamento da parte degli  \emph{stakeholder};
	...
	Una buona qualità dei processi induce una buona qualità del prodotto di questi. Per assicurarsi che un processo sia di qualità, è necessario un meccanismo di controllo che effettui modifiche sul \underline{\hyperref[sec:wow]{way of working}} ove ritenuto necessario sulla base di misurazioni della qualità dei processi. Per definire gli strumenti di valutazione della qualità dei processi dei fornitori in genere, non solo in ambito di sviluppo software, è stato messo a punto lo standard ISO 9000, in ambito specifico di ingegneria software si ha invece \underline{\hyperref[sec:iso90003]{ISO 90003:2004}}.
	...
	\subsection{Raintank}
	\label{sec:raintank}
	\emph{Raintank} è una piattaforma di monitoraggio \emph{open source} per raccogliere, archiviare e analizzare enormi quantità di dati diagnostici, da un'ampia varietà di fonti e in molti formati diversi.
	...
	\item Completi;
	\item Variabili;
	\item Consistenti: non si contraddicono a vicenda o da sè;
	\item Modificabili;
	\item Tracciabili;
	...
	\subsection{Riuso}
	\label{sec:riuso}
	Riutilizzo del codice. Può essere di due tipi: opportunistico (copia e incolla), che ha un basso costo e poco impatto, o sistematico, che ha un maggior costo e un maggior impatto.
	...
	\item Progettista: si occupa del \emph{design} del prodotto. In generale, si occupa della soluzione del problema;
	\item Programmatore: realizza il prodotto. Non ha libertà di scelta, segue le direttive del progettista;
	\item Verificatore: testa la qualità del prodotto e comunicano feedback;
	...
	Dove SV indica la \emph{schedule variance}, EV indica l'\emph{earned value} ovvero il valore del lavoro effettuato, PV indica il \emph{planned value} ovvero il costo del lavoro pianificato.
	...
	Se è positivo, l'attività è prevista terminare prima della scadenza quindi vi è un margine per eventuali imprevisti. Se è 0, o vicino, tale margine è piccolo o assente, si introducono quindi dei rischi. Se è negativo, la scadenza è passata e l'attività non è stata portata al termine.
	...
	Valutazione del valore dei costi svolta da una persona, secondo i propri criteri. Influenzata dalle dimensioni del progetto, dalle esperienze pregresse, dalla familiarità con le tecnologie adottate, dalla produttività dell'ambiente di lavoro, dalla qualità attesa. \\Un modello algoritmico che può aiutare in ciò è il \underline{\hyperref[sec:cocomo]{CoCoMo}}.
	...
	\subsection{Ticket}
	\label{sec:telegram}
	Un \emph{ticket} è l'associazione tra una task ed un assegnatario. Un sistema di \emph{ticketing} é una piattaforma per la gestione di \emph{task}, \emph{ticket} e assegnatari. 
	...
	Estensione di \emph{Javascript} incentrata nell'aumentarne la scalabilità. Sviluppata e manutenuta da Microsoft. Tra le \emph{feature} di interesse vi è il \emph{typing} statico e la possibilità di sviluppare applicazioni sia \emph{client-side} che \emph{server-side}.
	...
	\label{sec:wow}
	Insieme di regole, pratiche, che caratterizzano l'attività di sviluppo. Il \emph{way of working} è pensato per agevolare lo sviluppo ed evitare sprechi di risorse ed errori.
	
	
	\subsection{Webhook}
	\label{sec:webhook}
	Tecnologia nell'ambito del \emph{web development} che permette di modificare il comportamento di una pagina \emph{web} tramite \emph{callbacks} personalizzate.
	...
	\subsection{XML}
	\label{sec:xml}
	Si tratta di un meta linguaggio che permette di definire \underline{\hyperref[sec:linguaggiomarkup]{linguaggi di markup}}.
	...
	Termine inglese che significa "senza ritardo". Riferito al lavoro, indica un approccio che prevede l'inizio della attività di sviluppo non appena possibile. Permette di avere un margine di tempo tra termine del progetto e consegna.
	
	
	\subsection{Zero-laxity}
	\label{sec:zerolaxity}
	Termine inglese che significa "senza margine". Riferito al lavoro, indica un approccio che prevede l'inizio delle attività di sviluppo nel momento in cui la data di consegna del progetto meno il tempo richiesto dallo sviluppo equivale zero o meno.
