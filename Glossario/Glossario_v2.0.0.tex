...
\tableofcontents
	...
	\label{sec:airbnb}
	Insieme di convenzioni da rispettare per la produzione di codice \underline{\hyperref[sec:css]{CSS}} e \underline{\hyperref[sec:sass]{SASS}} di qualità, reperibile al seguente link \href{https://github.com/airbnb/css} {Airbnb Style guide}.
	...
	\label{sec:astahUML}
	\emph{Astah UML} è uno strumento di progettazione software gratuito che supporta gli ultimi diagrammi UML. La generazione del codice è disponibile con \underline{\hyperref[sec:plug-in]{plug-in}} gratuiti.
	...
	La \emph{Budget Variance} è una metrica che indica se la spesa \textit{complessiva} attuale è maggiore o minore alla spesa pianificata. Viene calcolata: $BV = EV - AC$, dove BV indica la \emph{budget variance}, EV indica l'\emph{earned value}, ovvero il valore delle attività realizzate, AC indica l'\emph{actual cost}, ovvero il costo effettivamente sostenuto.
	\subsection{Build}
	\label{sec:build}
	...
	Il \underline{\hyperref[sec:ciclodivita]{ciclo di vita}} di un \underline{\hyperref[sec:prodottosoftware]{software}} non è univocamente determinato, ma ne vengono individuati diversi modelli possibili. La scelta del modello dipende da 3 macro-fattori: cosa vuole il committente, dipendenza da terze parti, livello di coinvolgimento del committente nell'accertamento dello stato di avanzamento.
		...
		\subsubsection{Sequenziale (a cascata)}
			...
			Il modello fa forte uso di documentazione il che rende il sistema organizzato e tracciabile, prevede \emph{pre} e \emph{post} per ogni fase e associa ad ogni fase date di inizio e di fine.
ISO 12207 definisce così le fasi del ciclo di vita sequenziale: analisi, progettazione (intesa come \textit{design}), realizzazione, manutenzione.
		...
		\subsubsection{Incrementale}
			...
			\item L'uso di \emph{enablers} che facilitano il lavoro.
			\end{itemize}
		...
		Tali metodi offrono degli svantaggi poiché la documentazione è cruciale nella fase di manutenzione del \underline{\hyperref[sec:prodottosoftware]{software}} e l'assenza di pianificazione di fronte ai cambiamenti comporta dei rischi.
			...
			\subsubsection{Scrum}
				\emph{Scrum} ha come focus fondamentale le  \emph{user-stories}: una visione di ciò che vuole il cliente. Da esse si ricava un \emph{product backlog}, insieme di attività/feature da svolgere/realizzare. L'attività di \emph{Sprint Backlog} è un passo iterativo che impone controlli frequenti (\emph{Daily Scrum}) sulla correttezza delle azioni intraprese, tramite \emph{Sprint Review} e \emph{Retrospective}.
	...
	dove SV indica la \emph{schedule variance}, EV indica l'\emph{earned value}, ovvero il valore del lavoro effettuato e PV indica il \emph{planned value}, ovvero il costo del lavoro pianificato.
	...
	\subsection{Ticket}
	\label{sec:telegram}
	Un \emph{ticket} è l'associazione tra una \underline{\hyperref[sec:task]{task}} ed un assegnatario. Un sistema di ticketing è una piattaforma per la gestione di task, ticket e assegnatari.
	...
	\subsection{TravisCI}
	\label{sec:travisci}
	\emph{TravisCI} è un servizio distribuito per la gestione della Continuos Integration all'interno di repository \emph{GitHub}.
	
	\subsection{Trender}
	\emph{Trender} è un sistema che permette la gestione e il tracciamento di requisiti, casi d'uso, attori, verbali, package, classi e loro metodi.
	
	\subsection{TSLint}
	\label{sec:tslint}
	Strumento software per l'analisi statica del codice. Verifica che il codice rispetti un set di regole (estendibile) pensate per garantire conformità stilistica, evitare errori comuni, facilitare la manutenibilità, etc.. \emph{TSLint} è integrabile in svariati \emph{IDE} e nella piattaforma \underline{\hyperref[sec:travisci]{TravisCI}}.
