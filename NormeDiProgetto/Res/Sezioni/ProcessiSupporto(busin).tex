...
\begin{itemize}
	...
	\item \textbf{Inspection}: tecnica da applicare quando si ha idea delle possibili problematiche che si stanno cercando e si attua leggendo in modo mirato il codice sulla base di una lista di possibili errori, stilata in precedenza.
\end{itemize}
...
\begin{itemize}
	\item {\textbf{Test di unità}: verificano un'unità, il più piccolo sottosistema possibile (nella programmazione ad oggetti comunemente una classe o un metodo) che può essere testato separatamente;}
	\item {\textbf{Test di integrazione}: verificano se sono rispettati i contratti di interfaccia tra più moduli o sub-system;}
	\item {\textbf{Test di sistema}: verificano il comportamento dell'intero sistema software;}
	\item {\textbf{Test di regressione}: ogni modifica fatta al sistema scaturisce l'esecuzione di tutti i test effettuati in precedenza, per verificare il corretto comportamento del sistema a fronte di un cambiamento;}
	\item {\textbf{Test di accettazione}: collaudo svolto in presenza del richiedente che, se superato, porterà al rilascio del prodotto.}
\end{itemize}
